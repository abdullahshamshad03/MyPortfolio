        ...
        <TracingBeam className="px-6">
            ...
            <div className="max-w-2xl mx-auto antialiased pt-4 relative">
                {Content.map((item, index) => (
                    ...
                    <div key={`content-${index}`} className="mb-10 bg-[#232329]/50 border-white/80 border-2 p-4 shadow-md shadow-blue-500/50 hover:shadow-lg transition-shadow rounded-2xl  ">
                        <div className="flex justify-end ">
                        <h2 className=" text-white w-fit text-md font-semibold px-4 py-1 mb-1">
                            {item.badge}
                        </h2>
                        </div>
                       

                        <p className={twMerge(geistMono.className, "text-2xl text-[#18CCFC]/85 mb-4 font-bold ")}>
                            {item.title}
                        </p>

                        <div className="flex gap-3 items-center">
                            <p className=" text-lg font-semibold m-0"> {item.academics} </p>
                            <span className="border text-gray-300 w-fit text-xs px-2 py-[0.7px] rounded-full "> {item.duration} </span>
                        </div>
                        <div className="text-lg text-white/80 mt-5 prose prose-sm dark:prose-invert">
                            {item.description}
                        </div>
                    </div>
                ))}
